...
type Database struct {
	...
	FileVersionRepo    FileVersionRepositoryInterface
	DirectoryRepo      DirectoryRepositoryInterface
	BatchRepo          BatchRepositoryInterface
	QueryAnalyzer      QueryAnalyzerInterface
}
...
func NewDatabase(ctx context.Context, config *Config) (*Database, error) {
	...
	// Initialize all repositories with actual implementations
...
func (db *Database) BeginTransaction(ctx context.Context) (interface{}, error) {
	return db.Pool.Begin(ctx)
}

// ExecuteInTransaction executes a function within a database transaction
func (db *Database) ExecuteInTransaction(ctx context.Context, fn func(interface{}) error) error {
	tx, err := db.BeginTransaction(ctx)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}

	// Note: This is a simplified transaction handling
	// In a real implementation, you'd need proper type assertion and rollback handling
	defer func() {
		if r := recover(); r != nil {
			// Handle panic and rollback
			if tx != nil {
				// Type assert to call Rollback
				if txer, ok := tx.(interface{ Rollback(context.Context) error }); ok {
					txer.Rollback(ctx)
				}
			}
			panic(r)
	...
	// Commit on success
	if txer, ok := tx.(interface{ Commit(context.Context) error }); ok {
		if err := txer.Commit(ctx); err != nil {
...
func (db *Database) TestConnection(ctx context.Context) error {
	...
	_, err := db.FileRepo.Count(ctx, testUserID)
	if err != nil {
		return fmt.Errorf("file repository test failed: %w", err)
	}

	// Test query analyzer
	_, err = db.QueryAnalyzer.GetTableStats(ctx, "files")
	if err != nil {
		return fmt.Errorf("query analyzer test failed: %w", err)
	}

	log.Printf("Database connection test passed")
	return nil
}

// GetDatabaseInfo returns comprehensive database information
func (db *Database) GetDatabaseInfo(ctx context.Context) (*DatabaseInfo, error) {
	info := &DatabaseInfo{
	...
	if stats := db.GetStats(); stats != nil {
		info.TotalConnections = int(stats.TotalConns())
		info.IdleConnections = int(stats.IdleConns())
		info.AcquiredConnections = int(stats.AcquiredConns())
	}
...
// DatabaseInfo contains comprehensive database information
